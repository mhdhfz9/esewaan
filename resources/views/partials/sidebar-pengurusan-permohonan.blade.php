@php
    ...
    $listMenuBadgeLabel = auth()->user()->isAdminHq() && $hqPendingWithdrawal > 0
        ? 'Permohonan menunggu tindakan HQ termasuk tarik semula'
        : 'Permohonan baharu menunggu semakan';
@endphp
<details class="group/permohonan-mgmt [&::-webkit-details-marker]:hidden" {{ $permohonanMenuActive ? 'open' : '' }}>
    <summary class="sidebar-nav-summary {{ $permohonanMenuActive ? 'is-active' : '' }} [&::-webkit-details-marker]:hidden">
        ...
        @include('partials.sidebar-notification-badge', ['count' => $listMenuBadgeCount, 'label' => $listMenuBadgeLabel])
